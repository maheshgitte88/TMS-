...
export default function ItTms({tData}) {
  ...
  const calculateAverageTAT = (tickets) => {
    const resolvedTickets = tickets.filter(ticket => ticket.Resolution_Timestamp !== null);
    ...
    const totalTAT = resolvedTickets.reduce((total, ticket) => {
      const tat = getMinutesDifference(ticket.createdAt, ticket.Resolution_Timestamp);
      return total + tat;
    }, 0);
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        ...
        >
          <div>
            ...
            <h5 className="font-semibold">{calculateAverageTAT(tData).toFixed(3)}</h5>
          </div>
        </div>
